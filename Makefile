...
TEST_DIR=$(GOPATH)/src/github.com/statping-next/statping-next
...
lint:
	go fmt ./...
	golint ./...
	impi --local github.com/statping-next/statping-next/ --scheme stdLocalThirdParty ./...
	goimports ./...
...
lite: clean
	docker build -t statping-next/statping-next:dev -f dev/Dockerfile.dev .
...
docker-latest: 
	docker build -t statping-next/statping-next:latest --build-arg VERSION=${VERSION} .
...
dockerhub:
	docker build --build-arg VERSION=${VERSION} -t adamboutcher/statping-next:latest --no-cache -f Dockerfile .
	docker tag adamboutcher/statping-next adamboutcher/statping-next:v${VERSION}
	docker push adamboutcher/statping-next:v${VERSION}
	docker push adamboutcher/statping-next

docker-build-dev:
	docker build --build-arg VERSION=${VERSION} -t statping-next/statping-next:latest --no-cache -f Dockerfile .
	docker tag statping-next/statping-next:latest statping-next/statping-next:dev-v${VERSION}
...
download-bins: clean
	mkdir build || true
	wget "https://github.com/statping-next/statping-next/releases/download/v${VERSION}/statping-linux-386.tar.gz"
	wget "https://github.com/statping-next/statping-next/releases/download/v${VERSION}/statping-linux-amd64.tar.gz"
	wget "https://github.com/statping-next/statping-next/releases/download/v${VERSION}/statping-linux-arm.tar.gz"
	wget "https://github.com/statping-next/statping-next/releases/download/v${VERSION}/statping-linux-arm64.tar.gz"
...
buildx-latest: multiarch
	...
	docker buildx build --builder statping-latest --cache-from "type=local,src=/tmp/.buildx-cache" --cache-to "type=local,dest=/tmp/.buildx-cache,mode=max" --push --pull --platform linux/amd64,linux/arm64,linux/arm/v7,linux/arm/v6 -f Dockerfile -t adamboutcher/statping-next:latest -t adamboutcher/statping-next:v${VERSION} --build-arg=VERSION=${VERSION} --build-arg=COMMIT=${COMMIT} .
...
buildx-dev: multiarch
	...
	docker buildx build --builder statping-dev --cache-from "type=local,src=/tmp/.buildx-cache" --cache-to "type=local,dest=/tmp/.buildx-cache,mode=max" --push --platform linux/amd64,linux/arm64,linux/arm/v7,linux/arm/v6 -f Dockerfile -t adamboutcher/statping-next:dev --build-arg=VERSION=${VERSION} --build-arg=COMMIT=${COMMIT} .
