<template>
  <div>
    <div v-if="!embedded" class="card contain-card mb-5">
    ...
    <div v-else class="form-only">
      <form @submit="saveMessage">
          ...
          <div class="form-group row">
            <label for="notify_method_emb" class="col-sm-4 col-form-label">{{ $t('notify_users') }}</label>
            <div class="col-sm-8">
              <span @click="message.notify = !!message.notify" class="switch">
                <input v-model="message.notify" type="checkbox" class="switch" id="switch-normal-emb">
                <label for="switch-normal-emb">{{ $t('notify_desc') }}</label>
              </span>
            </div>
          </div>
          <div v-if="message.notify" class="form-group row">
          ...
          <div v-if="message.notify" class="form-group row">
            <label for="notify_before_emb" class="col-sm-4 col-form-label">{{ $t('notify_before') }}</label>
            <div class="col-sm-8">
              <div class="form-inline">
                <input v-model.number="message.notify_before" type="number" name="notify_before" class="col-4 form-control" id="notify_before_emb">
                <select v-model="message.notify_before_scale" class="ml-2 col-7 form-control" name="notify_before_scale" id="notify_before_scale_emb">
                  <option value="minute">{{ $t('minutes') }}</option>
                  <option value="hour">{{ $t('hours') }}</option>
                  <option value="day">{{ $t('days') }}</option>
                </select>
          ...
          <div class="alert alert-danger d-none" id="alerter_emb" role="alert"></div>
      </form>
    </div>
  </div>
</template>
