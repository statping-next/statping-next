<template>
    <div class="col-12">
        ...
        <p class="mb-3">{{ $t('tell_checkin_desc') }}</p>

        <div v-for="(checkin, i) in checkins" class="card mt-3">
            <div class="card-header text-capitalize">
                {{checkin.name}}
                <button @click="deleteCheckin(checkin)" class="btn btn-sm small btn-danger float-right text-uppercase">{{ $t('delete') }}</button>
            </div>
            <div class="card-body">
                <div class="input-group">
                    <input type="text" class="form-control" :value="`${core.domain}/checkin/${checkin.api_key}`" readonly>
                    <div class="input-group-append copy-btn">
                        <button @click.prevent="copy(`${core.domain}/checkin/${checkin.api_key}`)" class="btn btn-outline-secondary" type="button">{{ $t('copy') }}</button>
                    </div>
                </div>

                <span class="small">{{ $t('send_get_every_minutes', { interval: checkin.interval }) }}</span>
                <span class="small float-right mt-1 mr-3 d-none d-md-block">Requested {{ago(checkin.last_hit)}} ago</span>
                <span class="small float-right mt-1 mr-3 d-none d-md-block">{{ $t('request_expected_every_minutes', { interval: checkin.interval }) }}</span>

                <div class="card mt-3">
                    <div class="card-header text-capitalize">
                        <font-awesome-icon @click="expanded = !expanded" :icon="expanded ? 'minus' : 'plus'" class="mr-2 pointer"/>
                        {{checkin.name}} {{ $t('records') }}
                    </div>
                    <div class="card-body" :class="{'d-none': !expanded}">
                        <div class="alert alert-primary small" :class="{'alert-success': hit.success, 'alert-danger': !hit.success}" v-for="(hit, i) in records(checkin)">
                            Checkin {{hit.success ? $t('checkin_request') : $t('checkin_failure_label') }} at {{hit.created_at}}
                        </div>
                    </div>
                </div>
                ...
                <div class="card mt-3">
                    ...
                    <div class="card-body" :class="{'d-none': !curl_expanded}">
                        {{ $t('cronjob_script_desc', { interval: checkin.interval }) }}
                        <div class="input-group mt-2">
                            ...
                            <div class="input-group-append copy-btn">
                                <button @click.prevent="copy(`${checkin.interval} * * * * /usr/bin/curl ${core.domain}/checkin/${checkin.api_key} >/dev/null 2>&1`)" class="btn btn-outline-secondary" type="button">{{ $t('copy') }}</button>
                            </div>
                        </div>
                        <span class="small d-block">{{ $t('using_curl') }}</span>
                    </div>
                </div>
            </div>
            <div class="card-footer">
                <span :class="{'text-success': last_record(checkin).success, 'text-danger': !last_record(checkin).success}">
                    {{last_record(checkin).success ? $t('checkin_working') : $t('checkin_failing') }}
                </span>
            </div>
        </div>

        <div class="card mt-4">
            <div class="card-header text-capitalize">{{ $t('create_checkin') }}</div>
            <div class="card-body">
            <form @submit.prevent="saveCheckin">
                <div class="form-group row">
                    ...
                    <div class="col-5 col-md-3">
                        ...
                        <input v-model="checkin.interval" type="number" name="interval" class="form-control" id="checkin_interval" :placeholder="$t('checkin_interval_placeholder')" min="1">
                    </div>
                    <div class="col-12 col-md-4">
                        <label class="col-form-label"></label>
                        <button :disabled="btn_disabled" @click.prevent="saveCheckin" type="submit" id="submit" class="btn btn-primary d-block mt-2">{{ $t('save_checkin') }}</button>
                    </div>
                </div>
            </form>
            </div>
