  ...
  <div class="card shadow mb-4 global-incident-block incident-history-block" role="alert">
    <div class="card-body">
      ...
      <div class="global-incident-dates">
        <span class="global-incident-date-item">{{ $t('started_date') }} {{ niceDate(incident.created_at) }} ({{ aboutAgo(incident.created_at) }})</span>
        <span class="global-incident-date-sep">·</span>
        <span class="global-incident-date-item">{{ $t('last_updated_date') }} {{ niceDate(lastUpdatedTime(incident)) }} ({{ aboutAgo(lastUpdatedTime(incident)) }})</span>
      </div>
...
export default {
  ...
  computed: {
    serviceLabel() {
      return this.serviceName || this.$t('global_service')
    },
    isServiceLink() {
